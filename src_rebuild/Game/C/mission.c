...
int residentCarModels[MAX_CAR_RESIDENT_MODELS];
...
void SetupResidentModels()
{
	...
	if (MissionHeader->type & 1)
	{
		...
		if (PlayerStartInfo[0]->model > 4)
			residentCarModels[4] = PlayerStartInfo[0]->model;
	}
	...
	for(i = 0; i < 2; i++)
	{
		if (wantedCar[i] != -1)
		{
			...
			{		
				residentCarModels[4] = wantedCar[i];
			}
			else if(foundRM == -1)
			{
				residentCarModels[takenSlots++] = wantedCar[i];
			}
			else
			{
	...
	// [A] copy back to mission header for compatibility (if needed)
	for (i = 0; i < 5; i++)
		MissionHeader->residentModels[i] = residentCarModels[i];
}
...
void LoadMission(int missionnum)
{
	...
	// Try language-specific mission first
	sprintf(filename, "MISSIONS\\%s\\M%d.D2MS", LanguageNames[gUserLanguage], missionnum);
	...
	else
	{
		...
		sprintf(filename, "MISSIONS\\EN\\M%d.D2MS", missionnum);
...
#endif
		{
			// load from BLK (language-specific)
			sprintf(filename, "MISSIONS\\%s\\MISSIONS.BLK", LanguageNames[gUserLanguage]);
			
			// Fallback to English BLK if language-specific doesn't exist
			if (FileExists(filename) == 0)
				sprintf(filename, "MISSIONS\\EN\\MISSIONS.BLK");
	...
	if (MissionLoadAddress->route && !NewLevel)
		loadsize = (int)((long)((char*)MissionStrings + ((char*)MissionLoadAddress->route - (char*)MissionLoadAddress)));
	else
		loadsize = length;
