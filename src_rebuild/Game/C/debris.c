...
void PlacePoolForCar(CAR_DATA *cp, CVECTOR *col, int front, int in_car)
{
	...
	if (front) 
	{
		...
		if (in_car) 
		{
			// Use same geometry as normal mode
			s1[1].vz = s1[8].vz + 100;
			s1[0].vx = 136;
			s1[1].vx = -344;
			s1[2].vx = -21;
	...
	for(i = 0; i < count; i++)
	{
		...
		// [A] Clamp to prevent 16-bit overflow
		int view_x = toss.vx - camera_position.vx;
		int view_z = toss.vz - camera_position.vz;
		
		if (view_x > 32767) s[i].vx = 32767;
		else if (view_x < -32768) s[i].vx = -32768;
		else s[i].vx = view_x;
		
		if (view_z > 32767) s[i].vz = 32767;
		else if (view_z < -32768) s[i].vz = -32768;
		else s[i].vz = view_z;
		...
		{
			int view_y = -car_road_height - camera_position.vy;
			if (view_y > 32767) s[i].vy = 32767;
			else if (view_y < -32768) s[i].vy = -32768;
			else s[i].vy = view_y;
		}
		else
		{
			int view_y = -temp_y - camera_position.vy;
			if (view_y > 32767) s[i].vy = 32767;
			else if (view_y < -32768) s[i].vy = -32768;
			else s[i].vy = view_y;
		}
		...
		FIXEDH(s[0].vx * s[1].vy - s[0].vy * s[1].vx) * s[2].vz >= 0)
	{
		...
		if (sub_level == 0) 
	{
		// [A] Use 32-bit matrix operations first, then GTE for perspective
		SVECTOR sout_rear[4];
		VECTOR s_32bit;
		VECTOR result;
		
		// Transform all 4 vertices with rotation matrix
		for(int j = 0; j < 4; j++)
		{
			s_32bit.vx = s[j].vx;
			s_32bit.vy = s[j].vy;
			...
			ApplyMatrixLV(&inv_camera_matrix, &s_32bit, &result);
			
			if (result.vx > 32767) sout_rear[j].vx = 32767;
			else if (result.vx < -32768) sout_rear[j].vx = -32768;
			else sout_rear[j].vx = result.vx;
			
			if (result.vy > 32767) sout_rear[j].vy = 32767;
			else if (result.vy < -32768) sout_rear[j].vy = -32768;
			else sout_rear[j].vy = result.vy;
			
			if (result.vz > 32767) sout_rear[j].vz = 32767;
			else if (result.vz < -32768) sout_rear[j].vz = -32768;
			else sout_rear[j].vz = result.vz;
		}
		
		// Now use GTE only for perspective projection with identity matrix
		gte_SetRotMatrix(&identity);
		gte_SetTransVector(&dummy);
		
		gte_ldv3(&sout_rear[0], &sout_rear[1], &sout_rear[2]);
		gte_rtpt();
	...
	{
		// [A] Use 32-bit matrix operations to prevent GTE overflow
		VECTOR s_32bit;
		VECTOR result;
		
		for(i = 0; i < 12; i++)
		{
			// Convert SVECTOR to VECTOR
			s_32bit.vx = s[i].vx;
			s_32bit.vy = s[i].vy;
			s_32bit.vz = s[i].vz;
			
			// Transform using 32-bit precision
			ApplyMatrixLV(&inv_camera_matrix, &s_32bit, &result);
			
			// Clamp to 16-bit before converting back to SVECTOR
			if (result.vx > 32767) sout[i].vx = 32767;
			else if (result.vx < -32768) sout[i].vx = -32768;
			else sout[i].vx = result.vx;
			
			if (result.vy > 32767) sout[i].vy = 32767;
			else if (result.vy < -32768) sout[i].vy = -32768;
			else sout[i].vy = result.vy;
			
			if (result.vz > 32767) sout[i].vz = 32767;
			else if (result.vz < -32768) sout[i].vz = -32768;
			else sout[i].vz = result.vz;
		}
