...
int ProcessCarModelLump(char *lump_ptr, int lump_size)
{
	...
	for (i = 0; i < MAX_CAR_RESIDENT_MODELS; i++)
	{
		...
		if (model_number != -1)
		{
			...
			if (cleanOfs != -1)
			{
				if (lump_size > 0 && (cleanOfs >= lump_size || cleanOfs < 0))
				{
					printError("ProcessCarModelLump: cleanOfs %d out of bounds (size %d)\n", cleanOfs, lump_size);
				}
				else
				{
					mem = models_offset + cleanOfs;
					
					// [A] Validate model size before processing
					int mdlSize = ((MODEL*)mem)->poly_block; // KeepNormals=1 uses poly_block
					if (lump_size > 0 && (cleanOfs + mdlSize > lump_size || mdlSize < 0))
					{
						printError("ProcessCarModelLump: clean model size invalid (ofs %d, size %d, total %d)\n", cleanOfs, mdlSize, lump_size);
					}
					else
					{
						D_MALLOC_BEGIN();
			...
			// same for damaged and low detail
			if (damOfs != -1)
			{
				if (lump_size > 0 && (damOfs >= lump_size || damOfs < 0))
				{
					printError("ProcessCarModelLump: damOfs %d out of bounds (size %d)\n", damOfs, lump_size);
				}
				else
				{
					mem = models_offset + damOfs;
					
					// [A] Validate model size
					int mdlSize = ((MODEL*)mem)->normals; // KeepNormals=0 uses normals
					if (lump_size > 0 && (damOfs + mdlSize > lump_size || mdlSize < 0))
					{
						printError("ProcessCarModelLump: damaged model size invalid (ofs %d, size %d, total %d)\n", damOfs, mdlSize, lump_size);
					}
					else
					{
						D_MALLOC_BEGIN();
			...
			if (lowOfs != -1)
			{
				if (lump_size > 0 && (lowOfs >= lump_size || lowOfs < 0))
				{
					printError("ProcessCarModelLump: lowOfs %d out of bounds (size %d)\n", lowOfs, lump_size);
				}
				else
				{
					...
					// [A] Validate model size
					int mdlSize = ((MODEL*)mem)->poly_block; // KeepNormals=1 uses poly_block
					if (lump_size > 0 && (lowOfs + mdlSize > lump_size || mdlSize < 0))
					{
						printError("ProcessCarModelLump: low model size invalid (ofs %d, size %d, total %d)\n", lowOfs, mdlSize, lump_size);
					}
					else
					{
						D_MALLOC_BEGIN();
