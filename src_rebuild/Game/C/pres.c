...
void LoadFont(char *buffer)
{
	...
	char *file;
	RECT16 dest;
	static char *tempFontBuffer = NULL;
	...
	else
	{
		...
		// [A] On Android/non-PSX, _primTab2 might be NULL, so allocate a temporary buffer
		if (file == NULL)
		{
			if (tempFontBuffer == NULL)
			{
				// Allocate 128KB for font data (should be enough)
				tempFontBuffer = (char*)malloc(128 * 1024);
			}
			file = tempFontBuffer;
		}
	}
