...
void PrintCommandLineArguments()
{
	const char* argumentsMessage =
		...
		"  -ini <filename.ini> : starts game with specific filter ini\n"
...
int main(void)
#else
extern "C" int redriver2_main(int argc, char** argv)
#endif // PSX
{
	char** ScreenNames;
	...
	printf("malloctab = %x\n", malloctab);
#else
	// On non-PSX platforms, allocate memory buffers dynamically
	// Using same sizes as macOS/PC (see redriver2_psxpc.cpp)
	_frontend_buffer = (char*)malloc(0x50000);  // 320KB
	_other_buffer = (char*)malloc(0x50000);     // 320KB
	_other_buffer2 = (char*)malloc(0x50000);    // 320KB
	_OT1 = (OTTYPE*)malloc(0x4200 * sizeof(OTTYPE));
	_OT2 = (OTTYPE*)malloc(0x4200 * sizeof(OTTYPE));
	_primTab1 = (char*)malloc(0x1E000);         // 120KB
	_primTab2 = (char*)malloc(0x1E000);         // 120KB
	_overlay_buffer = (char*)malloc(0x3C000);   // 240KB
	_replay_buffer = (char*)malloc(0x16144);    // ~88KB
	_sbank_buffer = (char*)malloc(0x40000);     // 256KB
	malloctab = (char*)malloc(0x100000);        // 1MB
	mallocptr = malloctab;
	
#ifdef __ANDROID__
	#include <android/log.h>
	__android_log_print(ANDROID_LOG_INFO, "REDRIVER2", "Allocated game buffers (macOS-compatible sizes)");
#endif
#endif
...
#ifdef __ANDROID__
	#include <android/log.h>
	__android_log_print(ANDROID_LOG_INFO, "REDRIVER2", "Files verified, language initialized");
#endif
...
#ifdef __ANDROID__
		__android_log_print(ANDROID_LOG_INFO, "REDRIVER2", "Skipping splash screens on Android");
		// Skip splash screens on Android - they cause hang
#else
...
#ifdef __ANDROID__
	__android_log_print(ANDROID_LOG_INFO, "REDRIVER2", "Loading frontend");
#endif
	// Init frontend
	LOAD_OVERLAY("FRONTEND.BIN", _overlay_buffer);

#ifdef __ANDROID__
	__android_log_print(ANDROID_LOG_INFO, "REDRIVER2", "Frontend loaded, unmuting sound");
#endif
	SpuSetMute(0);
...
#ifdef __ANDROID__
	__android_log_print(ANDROID_LOG_INFO, "REDRIVER2", "Sound initialized, setting up score tables");
#endif
...
#ifdef __ANDROID__
	__android_log_print(ANDROID_LOG_INFO, "REDRIVER2", "Entering main game loop");
#endif
