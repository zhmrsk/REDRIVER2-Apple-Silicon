...
{
	// [A] Disabled PVS culling - always visible
	return 1;
...
void DrawAllTheCars(int view)
{
	...
	if (num_cars_to_draw != 0)
	{
		...
		for (i = 0; i < num_cars_to_draw; i++)
		{
			...
			if ((long)(current->primtab + (-3000 - (long)(current->primptr - PRIMTAB_SIZE))) < 5800)
				return;
...
void DrawMapPSX(int* comp_val)
{
	...
	do
	{
		// [A] Disabled PVS culling - render everything
		if (ABS(hloop) + ABS(vloop) < PVS_CELL_COUNT || 1) // Always true
		{
			// clamped vis values
			...
			if (drawData.rightPlane < 0 &&
				...
				//PVS_ptr[vis_v * pvs_square + vis_h]) // check PVS table  [A] Disabled
				1) // Always render
			{
