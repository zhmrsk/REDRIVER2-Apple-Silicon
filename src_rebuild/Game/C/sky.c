...
// Skybox rendering flag for depth control
extern int g_renderingSkybox;
...
void PlotHorizonMDL(MODEL* model, int horizontaboffset, RGB16* skycolor)
{
	...
	// Enable depth test back (depthTest = 1) - Added FIRST so it executes LAST
	{
		DR_PSYX_DEPTHTEST* depthEnable = (DR_PSYX_DEPTHTEST*)current->primptr;
		SetPsyXDepthTest(depthEnable, 1);
		addPrim(current->ot + OTSIZE - 1, depthEnable);
		current->primptr += sizeof(DR_PSYX_DEPTHTEST);
	}

	// Enable Z-write back (added FIRST so it executes LAST)
	{
		DR_PSYX_ZWRITE* zEnable = (DR_PSYX_ZWRITE*)current->primptr;
		SetPsyXZWrite(zEnable, 1);
		addPrim(current->ot + OTSIZE - 1, zEnable);
		current->primptr += sizeof(DR_PSYX_ZWRITE);
	}
	...
	// Disable Z-write (added LAST so it executes FIRST)
	{
		DR_PSYX_ZWRITE* zDisable = (DR_PSYX_ZWRITE*)current->primptr;
		SetPsyXZWrite(zDisable, 0);
		addPrim(current->ot + OTSIZE - 1, zDisable);
		current->primptr += sizeof(DR_PSYX_ZWRITE);
	}

	// Enable SKYBOX MODE (depthTest = 2) - Added LAST so it executes FIRST
	{
		DR_PSYX_DEPTHTEST* depthDisable = (DR_PSYX_DEPTHTEST*)current->primptr;
		SetPsyXDepthTest(depthDisable, 2); // 2 = Skybox Mode (glDepthRange forced)
		addPrim(current->ot + OTSIZE - 1, depthDisable);
		current->primptr += sizeof(DR_PSYX_DEPTHTEST);
	}
}
...
void DrawSkyDome(void)
{
	...
	{
		PlotHorizonMDL(modelpointers[1], HorizonLookup[GameLevel][2], &skycolor);
		PlotHorizonMDL(modelpointers[3], HorizonLookup[GameLevel][3], &skycolor);
	}
#else
	PlotHorizonMDL(modelpointers[0], HorizonLookup[GameLevel][0], &skycolor);
	PlotHorizonMDL(modelpointers[1], HorizonLookup[GameLevel][2], &skycolor);
