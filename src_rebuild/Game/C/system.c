...
int Loadfile(char* name, char* addr)
{
	int nread;
	unsigned char res[8];
	char namebuffer[256];  // Increased from 64 to 256 for long Android paths
...
int FileExists(char* filename)
{
	...
	__android_log_print(ANDROID_LOG_INFO, "REDRIVER2", "FileExists: checking '%s'", namebuffer);
#else
	printf("FileExists: checking '%s'\n", namebuffer);
#endif
...
{
	char namebuffer[256];  // Increased from 64 to 256
