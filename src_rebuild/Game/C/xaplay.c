...
void StoreXASubtitles()
{
	...
	for (i = 0; i < 4; i++)
	{
		for (j = 0; j < 8; j++)
		{
			...
			{
				// Save subtitles file
				sprintf(fileName, "%sXA\\%s_XABNK0%d.XA[%d].SBN", gDataFolder, LanguageNames[gUserLanguage], i+1, j);
				FS_FixPathSlashes(fileName);
...
void PlayXA(int num, int index)
{
	...
	if (xa_prepared && gPlaying != 1)
	{
		...
		sprintf(fileName, "%sXA\\%s_XABNK0%d.XA[%d].SBN", gDataFolder, LanguageNames[gUserLanguage], num + 1, index);
		FS_FixPathSlashes(fileName);
