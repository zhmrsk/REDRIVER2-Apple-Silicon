...
void FEGetHiresBakedQuad(int char_index, float scale, float* xpos, float* ypos, FEFONT_QUAD* q)
{
	...
	int r = 0;
	for (int i = 0; i < gHiresFEFontRangeCount; i++)
	{
		if (char_index >= gHiresFEFontRanges[i].start && char_index < gHiresFEFontRanges[i].start + gHiresFEFontRanges[i].count)
		{
			r = i;
			break;
		}
	...
	const OUT_FN2INFO* b = gHiresFEFontCharData[r] + char_index - gHiresFEFontRanges[r].start;
...
void LoadFrontendScreens(int full)
{
	extern void RefreshMenuStrings(void);
...
void RefreshMenuStrings(void)
{
	static const MenuStringMap mapping[] = {
		{0, 0, GTXT_Undercover},
		{0, 1, GTXT_TakeARide},
		{0, 2, GTXT_DrivingGames},
		{0, 3, GTXT_MultiplayerMenu},
		{0, 4, GTXT_Options},
		{0, 5, GTXT_Replays},
		...
		{2, 0, GTXT_NewGame},
		{2, 1, GTXT_LoadGame},
		{2, 2, GTXT_ReplayMission},
		{2, 3, GTXT_ViewCutscenes},
		...
		{5, 0, GTXT_QuickChase},
		{5, 1, GTXT_QuickGetaway},
		{5, 2, GTXT_GateRacing},
		{5, 3, GTXT_Trailblazer},
		{5, 4, GTXT_Checkpoint},
		{5, 5, GTXT_Survival},
		
		{6, 0, GTXT_TakeARide},
		{6, 1, GTXT_CopsNRobbers},
		{6, 2, GTXT_Checkpoint},
		{6, 3, GTXT_CaptureTheFlag},
		...
		{12, 0, GTXT_Accept},
		{12, 1, GTXT_UseDirectionalButtons},
		{12, 2, GTXT_ToMoveScreen},
		
		{13, 0, GTXT_Sound},
		{13, 1, GTXT_Gameplay},
		{13, 2, GTXT_HighScores},
		{13, 3, GTXT_CenterScreen},
		{13, 4, GTXT_SaveSettings},
		{13, 5, GTXT_LoadSettings},
		
		{14, 0, GTXT_Previous},
		{14, 1, GTXT_Select},
		{14, 2, GTXT_Next},
		{14, 3, GTXT_ChooseARide},
		
		{15, 0, GTXT_CopDifficulty},
		{15, 1, GTXT_Subtitles},
		{15, 2, GTXT_Vibration},
		{15, 3, GTXT_Controller},
		{15, 4, GTXT_Secret},
		...
		{19, 0, GTXT_Previous},
		{19, 1, GTXT_SurveillanceTipOff},
		{19, 2, GTXT_AutoTesting},
		{19, 3, GTXT_AutoTesting},
		{19, 4, GTXT_LennyGetsCaught},
		{19, 5, GTXT_Next},
		...
		{21, 2, GTXT_Vegas},
		{21, 3, GTXT_Rio},
		{21, 4, GTXT_DirectorsCut},
		
		{22, 0, GTXT_Previous},
		{22, 1, GTXT_Select},
		{22, 2, GTXT_Next},
		{22, 3, GTXT_ChooseACutscene},
		
		{23, 0, GTXT_Sfx},
		{23, 1, GTXT_Music},
		{23, 2, GTXT_SetVolumeLevels},
		{23, 3, GTXT_Language},
		{23, 4, GTXT_Accept},
		...
		{25, 0, GTXT_SinglePlayer},
		{25, 1, GTXT_Multiplayer},
		
		{26, 0, GTXT_QuickGetaway},
		{26, 1, GTXT_GateRacing},
		{26, 2, GTXT_Trailblazer},
		{26, 3, GTXT_Checkpoint},
		{26, 4, GTXT_Survival},
		
		{27, 0, GTXT_On},
		{27, 1, GTXT_Off},
		
		{28, 0, GTXT_AutoTesting},
		
		{29, 0, GTXT_MountainPass},
		{29, 1, GTXT_RaceTrack},
		{29, 2, GTXT_Invincibility},
		{29, 3, GTXT_Immunity},
		...
		{33, 0, GTXT_SinglePlayer},
		{33, 1, GTXT_Multiplayer},
		...
		{35, 0, GTXT_RiverNorth},
		{35, 1, GTXT_CabriniGreen},
		
		{36, 0, GTXT_PlazaDeRevolucion},
		{36, 1, GTXT_OldHavana},
	...
	{
		strcpy(PsxScreens[mapping[i].screenId].buttons[mapping[i].buttonId].Name, G_LTXT(mapping[i].strId));
	}
}
...
int SetVolumeScreen(int bSetup)
{
	...
	if (bSetup) 
	{
		...
		FEPrintString(LanguageNames[gUserLanguage], 152, ypos[2], 2, 128, 128, 128);
#endif
	...
	currSelIndex = pCurrButton - pCurrScreen->buttons;
	...
	if (feNewPad & MPAD_TRIANGLE)
	{
		...
		gUserLanguage = languageHold;

		// Revert strings if language changed back
		DeinitStringMng();
		InitStringMng();
		RefreshMenuStrings();
	...
	{
		if (currSelIndex == 4) // Accept (Index 4)
			LoadBackgroundFile("DATA\\GFX.RAW");
	...
	else
	{
		...
		if (fePad & MPAD_D_LEFT)
		{
			...
			case 1:
				...
				break;
			case 3: // Language
				if(feNewPad & MPAD_D_LEFT) // Only once per press
				{
					gUserLanguage--;
					if (gUserLanguage < 0)
						gUserLanguage = 5;
		...
		else if (fePad & MPAD_D_RIGHT)
		{
			...
			case 1:
				...
				break;
			case 3: // Language
				if(feNewPad & MPAD_D_RIGHT) // Only once per press
				{
					gUserLanguage++;
					if (gUserLanguage > 5)
						gUserLanguage = 0;
	...
	if (update)
	{
		...
		{
			FEPrintString(LanguageNames[gUserLanguage], 152, ypos[2], 2, 128, 128, 128);
			FEPrintString(LanguageNames[gUserLanguage], 152, ypos[2], 2, 0, 0, 0); // Shadow? Or clear? 
		}
		else
		{
			sprintf(text, "%d", (int)(10000 + last) / 100);
			int y = (currSelIndex == 1) ? ypos[1] : ypos[0];
			FEPrintString(text, 152, y, 2, 128, 128, 128);
			FEPrintString(text, 152, y, 2, 0, 0, 0);
		}
