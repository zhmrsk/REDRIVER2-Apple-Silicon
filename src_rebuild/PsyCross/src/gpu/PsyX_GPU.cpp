...
int g_zWriteEnabled = 1;
int g_depthTestEnabled = 1;
int g_renderingSkybox = 0;  // Flag to control skybox-specific rendering
...
static void AddSplit(bool semiTrans, bool textured)
{
	...
	if (curSplit.blendMode == blendMode &&
		...
		curSplit.zWrite == g_zWriteEnabled &&
		curSplit.depthTest == g_depthTestEnabled)
	{
		return;
	}
	...
	split.zWrite = g_zWriteEnabled;
	split.depthTest = g_depthTestEnabled;
...
void DrawSplit(const GPUDrawSplit& split)
{
	...
	// Check if rendering skybox (depthTest == 2) - override depth settings
	if (split.depthTest == 2) {
		glDepthMask(GL_FALSE);  // Don't write to Z-buffer
		glDepthFunc(GL_LEQUAL); // Standard test
		glDepthRange(0.99999f, 1.0f); // Force all depth values to be at the very far plane
	} else {
		glDepthRange(0.0f, 1.0f); // Restore normal depth range
		PsyX_SetZWriteEnable(split.zWrite);
...
static int ProcessPsyXPrims(P_TAG* polyTag)
{
	...
	case 0x03:
	{
		// [A] Psy-X custom Z-write packet
		DR_PSYX_ZWRITE* psyz = (DR_PSYX_ZWRITE*)polyTag;
		g_zWriteEnabled = psyz->enable;
		return 3;  // code[2] + enable = 3 uints
	}
	case 0x04:
	{
		// [A] Psy-X custom depth test packet
		DR_PSYX_DEPTHTEST* psyd = (DR_PSYX_DEPTHTEST*)polyTag;
		g_depthTestEnabled = psyd->enable;
		return 3;  // code[2] + enable = 3 uints
	}
	}
