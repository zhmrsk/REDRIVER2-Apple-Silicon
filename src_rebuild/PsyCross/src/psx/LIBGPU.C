...
void SetPsyXZWrite(DR_PSYX_ZWRITE* p, int enable)
{
	setlen(p, sizeof(DR_PSYX_ZWRITE) / 4 - P_LEN);
	p->code[0] = 0xB3000000; // Opcode in MSB for P_TAG.code access
	p->code[1] = 0;
	p->enable = enable;
}

void SetPsyXDepthTest(DR_PSYX_DEPTHTEST* p, int enable)
{
	setlen(p, sizeof(DR_PSYX_DEPTHTEST) / 4 - P_LEN);
	p->code[0] = 0xB4000000; // Opcode in MSB for P_TAG.code access
	p->code[1] = 0;
	p->enable = enable;
}
