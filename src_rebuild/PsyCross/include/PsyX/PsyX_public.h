...
/* Sets Z-write enable state */
extern void PsyX_SetZWriteEnable(int enable);

#include <stdint.h>

typedef struct {
	uintptr_t addr;
	unsigned int len_pgxp; // len : 16, pgxp_index : 16
	unsigned int code[2];
	int enable;
} DR_PSYX_ZWRITE;

extern void SetPsyXZWrite(DR_PSYX_ZWRITE* p, int enable);
...
extern void SetPsyXDepthTest(DR_PSYX_DEPTHTEST* p, int enable);
