...
void DoPlayFMV(RENDER_ARG* arg, int subtitles)
{
	...
	char filename[250];
	snprintf(filename, sizeof(filename), "%sFMV\\%d\\RENDER%d.STR[0].avi", gDataFolder, fd, arg->render);
	...
	if (subtitles)
	{
		snprintf(filename, sizeof(filename), "%sFMV\\%d\\%s_RENDER%d.SBN", gDataFolder, fd, LanguageNames[gUserLanguage], arg->render);
		FS_FixPathSlashes(filename);
	...
	if(arg->credits)
	{
		snprintf(filename, sizeof(filename), "%sDATA\\CREDITS.ENG", gDataFolder);
		FS_FixPathSlashes(filename);
