...
--
-- "uses" do not respect filters/filters due to havy optimizations
-- "usage" can contain filter filters
...
-- * if "usage" is already on stack it mean that "usage" in this filter was in use, 
--
	...
	local function resolveUses( sourceProject, targetProject, targetBlock )
		...
		if optimization then
			...
			if sourceProject.ctx.usesValues == nil then
				sourceProject.ctx.usesValues = context.fetchvalue( sourceProject.ctx, 'uses_fix' )
			end 
...
--
	...
	local function resolveAllUsesInBlock( targetProject, targetBlock )		
		...
		for _, usageKey in ipairs(targetBlock.uses_fix) do
