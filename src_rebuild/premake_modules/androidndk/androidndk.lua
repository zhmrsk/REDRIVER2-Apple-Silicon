...
-- Returns:
-- * The ABI name for supported filters.
-- * "all" (like the "all" that can be passed to APP_ABI) if the filter doesn't have an ABI filter.
-- * nil for unsupported filters.
function androidndk.getConfigAbi(config)
	...
	-- If needed, it can be exposed via a separate filter variable.
...
function androidndk.isConfigSupported(config, reportWarnings)
	if config.flags.ExcludeFromBuild then
		...
		-- Warnings specific to this module shouldn't be reported either as the filter may not be targeting this module at all.
		return false;
	end
	if config.project ~= nil then
		assert(androidndk.isProjectSupported(config.project, false), "Usage of filters for unsupported projects must not be attempted at all");
	end
	...
	if reportWarnings then
		if config.project ~= nil then
			warningLocation = string.format("workspace '%s' project '%s' filter '%s'", config.workspace.name, config.project.name, config.name);
		else
			warningLocation = string.format("workspace '%s' filter '%s'", config.workspace.name, config.name);
		end
	end
	local configSupported = true;
	-- Whitespaces because filters are placed in a whitespace-separated list.
	-- % because filters and platforms are used in filter and filter-out, where % is a wildcard.
	...
	if androidndk.staticallyHasPatternPostEsc(p.esc(config.buildcfg), disallowedPattern) then
		...
		if reportWarnings then
			p.warn(
				"Skipping %s with filter name containing whitespaces or disallowed characters %s",
