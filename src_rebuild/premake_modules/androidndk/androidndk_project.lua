...
function androidndk.getConfigCondition(buildcfg, platform, abi)
	-- All requirements must be met by what is provided by ndk-build and the user for a filter to be selected.
	...
	-- PREMAKE_ANDROIDNDK_CONFIGURATIONS and _PLATFORMS include _all_ needed filters and are the same throughout the ndk-build execution.
	...
	-- The result for multiple filters for one ABI is undefined.
	...
	-- At least one build filter is required by Premake.
	table.insert(state, "$(PREMAKE_ANDROIDNDK_CONFIGURATIONS_PREFIXED)");
	table.insert(requirements, "CONFIGURATION=" .. p.esc(buildcfg));

	-- Platforms may be unspecified, in this case, filters will have a nil platform.
...
-- In this module, flags are gathered from both the project filter and per-file filter.
-- ndk-build, however, doesn't support per-file flags.
-- Per-file filter is needed though because compilers for different languages may need different flags.
	...
	for i, settingName in ipairs(androidndk.includeDirsConfigSettings) do
		...
		for i2, includeDir in ipairs(settingInput) do
			...
			if androidndk.staticallyHasPatternPostEsc(includeDirPath, "[%s#%$]") then
				p.warn(
					"Skipping workspace '%s' project '%s' filter '%s' include directory '%s' with whitespaces or disallowed characters #$",
...
function androidndk.addCompilerFlagsForExtension(flagsForExtension, extension, config, temporaryVariables)
	...
	if extension == androidndk.asmExtension then
		for i, forceInclude in ipairs(config.forceincludes) do
			...
			if androidndk.staticallyHasPatternPostEsc(forceIncludePath, "[\"';]") then
				p.warn(
					"Skipping workspace '%s' project '%s' filter '%s' Yasm force include '%s' with path containing disallowed characters \"';",
	...
	elseif isCCpp then
		...
		for i, forceInclude in ipairs(config.forceincludes) do
			...
			if androidndk.staticallyHasPatternPostEsc(forceIncludePath, "[\"]") then
				p.warn(
					"Skipping workspace '%s' project '%s' filter '%s' C/C++ force include '%s' with path containing a disallowed double quote character",
					config.workspace.name, config.project.name, config.name, forceInclude);
			else
				table.insertkeyed(
					flagsForExtension.buildoptions,
	...
	-- Flags for filter flags setting.
...
function androidndk.generateProjectConfig(config)
	...
	-- At this point, it's needed for getting the file filter for it so, if needed, C++-specific options can be extracted from it.
	local pch = nil;
	if not config.flags.NoPCH and config.pchheader ~= nil then
		...
		if androidndk.staticallyHasPatternPostEsc(pch, "[%s\"#%$&'%(%),;<=>`|]") then
			p.warn(
				"Skipping workspace '%s' project '%s' filter '%s' precompiled header '%s' " ..
	...
	p.tree.traverse(sourceTree, {
		onleaf = function(fileNode, depth)
			...
			-- Check if the file is present at all in the current filter and not explicitly excluded with a file filter.
			if fileConfig ~= nil then
				...
				-- If this is the precompiled header, store the file filter for it so it can be used later to obtain C++-specific settings.
					...
					if sourceOriginalBaseName ~= "" then
						if sourceOriginalExtension == ".mk" then
							...
							if androidndk.staticallyHasPatternPostEsc(sourceAndroidMkPath, "[%s]") then
								p.warn(
									"Skipping workspace '%s' project '%s' filter '%s' file '%s' " ..
										...
										if androidndk.staticallyHasPatternPostEsc(sourceOriginalRelativePath, "[%s\"#%$&'%(%),;<>`|]") then
											...
											if sourceExtension == androidndk.asmExtension then
												...
												if configAbiSupportsYasm then
													if configAbi == androidndk.ABI_ALL then
														-- Add YASM sources conditionally for multi-ABI filters.
											...
											if sourceFileTable ~= nil and sourceFileTable[sourceRelativePath] == nil then
												...
												if compilerFlagsVariable ~= nil then
													...
													-- Add non-extension-specific compiler options from the project filter if hasn't done already.
	...
	for i, file in ipairs(sourcePrecreatedFiles) do
		p.warn(
			"Workspace '%s' project '%s' filter '%s' contains an external Android.mk or a prebuilt library '%s' among its source files, " ..
	...
	if fltoCOverride ~= nil and fltoCOverride ~= fltoBuildOption then
		p.warn(
			"Workspace '%s' project '%s' filter '%s' uses LTO type '%s' for C, while it uses '%s' for another language",
	...
	if isLinked then
		...
		while ldDependencyIndex <= #ldDependencies do
			...
			if ldDependency ~= config and ldDependencyExplicitFlto ~= nil then
				if ldFlagsExplicitFlto ~= nil and ldFlagsExplicitFlto ~= ldDependencyExplicitFlto then
					p.warn(
						"Workspace '%s' project '%s' filter '%s' has mismatching -flto link options " ..
		...
		if ldLocalFlagsExplicitFlto ~= nil then
			...
			if ldFlagsExplicitFlto ~= nil and ldFlagsExplicitFlto ~= ldLocalFlagsExplicitFlto then
				p.warn(
					"Workspace '%s' project '%s' filter '%s' has a -flto link option '%s' " ..
		...
		if fltoBuildOption ~= nil and fltoBuildOption ~= ldFlagsFlto then
			if ldFlagsFlto ~= nil then
				p.warn(
					"Workspace '%s' project '%s' filter '%s' uses a -flto build option '%s' " ..
	...
	-- Gather module dependencies for the current build filter and platform.
	...
	if abiCFlagsConditionalOpen then
		assert(configAbi == androidndk.ABI_ALL, "Can have ABI conditionals for ABI-specific C flags only for ABI-agnostic filters");
		p.w("endif");
	end
	...
	if androidndk.staticallyHasPatternPostEsc(moduleFileName, "[%s#%$%%/;|]") then
		-- % needs to be escaped as %% in printf format strings.
		p.warn(
			"Using the default build target name for workspace '%s' project '%s' filter '%s' " ..
	...
	-- Single-ABI filters are already handled while gathering the source file nodes.
	-- Write the assignment for multi-ABI filters.
...
function androidndk.onProject(project)
	...
	p.generate(project, ".prj.Android.mk", function(project)
		...
		-- Gather valid filters for each ABI.
		-- ABI-agnostic filters are checked last.
		...
		for config in p.project.eachconfig(project) do
			if androidndk.isConfigSupported(config, true) then
				...
				if abisConfigs[configAbi] == nil then
					...
					-- The ABI-agnostic filters will be used as fallbacks when no ABI specialization is available.
