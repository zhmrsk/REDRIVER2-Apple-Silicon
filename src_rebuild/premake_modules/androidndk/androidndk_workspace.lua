...
-- Projects can add their own filters and platforms.
-- So, the use of workspace.filters, workspace.projects, and their product workspace.configs, should be avoided.
-- Instead, configs should be gathered from all projects.

-- Writes the setup of the variables used in filter conditionals.
...
function androidndk.setupConfigConditionalMakeVariables(usedBuildcfgs, usedPlatforms)
	...
	p.w("$(warning No filters to build are specified in PREMAKE_ANDROIDNDK_CONFIGURATIONS.)");
	p.w("$(warning Specify one or multiple \"PREMAKE_ANDROIDNDK_CONFIGURATIONS+=filter\" in the ndk-build arguments to build those filters.)");
	...
	p.w("$(warning Note that filter and platform names are case-sensitive in this script.)");
	p.w("$(warning )");
	p.w("$(warning It's heavily recommended that only at most one filter and platform specified corresponds to each targeted ABI.)");
	p.w("$(warning Otherwise, which set of project settings will actually be chosen for building for each ABI will be undefined.)");
	p.w("$(warning )");
	p.w("$(warning The recommended approach is to use filters for the optimization mode, and, if needed, platforms for ABI filtering.)");
	...
	p.w("$(warning In this case, each ABI will unambiguously correspond to only one filter and platform pair.)");
	p.w("$(warning )");
	...
	-- Generate variables used for filter selection.
...
function androidndk.findConfigForAppSetting(workspace, buildcfg, platform, predicate)
	-- In none of these checks should completely unrequested build filters or platforms pass.
	...
	-- This filter may be used by just a subset of the projects.
...
function androidndk.gatherEscapedAppVariables(workspace, buildcfg, platform)
	-- If the value for a Premake setting is not specified by the projects for this filter, the default must not be added to the table.
	-- This would make it impossible for another filter, where the setting is explicitly defined, to override the value using ?=.
	-- Only returning variables corresponding to settings explicitly defined for this filter.
	...
	if optimizeConfig ~= nil then
		-- The filter explicitly wants to enable or to disable optimizations.
	...
	if stlConfig ~= nil then
		-- The default, which in ndk-build is "none", is explicitly requested for this filter.
...
function androidndk.onWorkspace(workspace)
	...
	-- Gather build filters and platforms used by all projects for generating conditionals for application settings.
	-- Projects may add their filters not present in the workspace itself, and they won't appear in workspace.configs.
	...
	-- At least one build filter is required in a project, however.
	local usedBuildcfgs = {};
	local usedPlatforms = {};
	for project in p.workspace.eachproject(workspace) do
		if androidndk.isProjectSupported(project, false) then
	...
	p.generate(workspace, ".Application.mk", function(workspace)
		...
		-- Gather Application.mk variable values for selected filters and platforms.
		-- As this is Application.mk, ABI filtering is not possible.
		-- Therefore, selection of the required build filters and platforms will work differently than in projects.
		-- All selected build filters and platforms must have the same values of settings written here.
		-- For example, APP_OPTIM may be wrong with `PREMAKE_ANDROIDNDK_CONFIGURATIONS := Debug Release` if they have different `optimize`.
		-- If one selected filter-platform defines a value, while another doesn't care, pick from the one where it's explicitly set.
		-- So, using if/if and ?= instead of if/elseif and :=, unlike in projects (where only filter-platform pair is supposed to be chosen).
		-- Most APP variables accept only one value - using ?= for them.
		-- APP_ABI, however, accumulates the needed ABIs (in this case, for selected filters and platforms), so using += for it.
		...
		-- Gather ABIs the projects are built for, for each filter and platform for it.
		-- Can't just collect all ABIs used by all platforms into a single APP_ABI assignment.
		-- The reason is that some filters may not even be targeting Android.
		...
		for i, buildcfg in ipairs(usedBuildcfgs) do
			...
			-- Skip ABIs needed by platform-agnostic projects with this filter from platform specializations.
		...
		-- Apply defaults if no filter-specific overrides have been chosen, and normalize variables.
		-- Also set variables that don't depend on the filter.

		-- APP_ABI
		-- Setting to "all" by default is not needed - empty already means `all`.
		-- Also, it can happen only when no filter with projects to build is selected.
		-- Remove duplicates (sorting does that implicitly).
		p.w("APP_ABI := $(sort $(APP_ABI))");
		-- If "all" was set for any filter, and += of any other ABIs happened, make APP_ABI just "all".
