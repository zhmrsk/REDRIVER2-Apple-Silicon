...
## Fork Features (macOS ARM64 & Ukrainian Localization)

This fork includes significant enhancements over the original project:

### 🇺🇦 Ukrainian Localization
- Full translation of all menus, missions, and subtitles
- 12 language files, 258 subtitle files, 534 mission scripts
- Localization tools included in `tools/localization/`

### 🍎 macOS ARM64 Native Support
- Native support for Apple Silicon (M1/M2/M3)
- Optimized for macOS 11.0+ (Big Sur through Sequoia)
- Reduced app bundle size (27MB vs 73MB+)
- Custom Launcher with system Java detection

### 🔧 Improvements
- **64-bit Architecture**: Modified PsyCross library for modern 64-bit systems
- **Headlights Fix**: Correct rendering in first-person view
- **Language Loading**: Improved fallback mechanism
### Dependencies
...
To create a PKG installer for distribution:

```bash
./package_release.sh
```

This creates `release/REDRIVER2_Installer.pkg` (~11MB). 
**Note:** The installer is unsigned. Users must Right-Click -> Open to install.
The postinstall script automatically fixes permissions.
